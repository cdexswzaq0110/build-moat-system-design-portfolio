...
- Optional MCP smoke test: developer inspector requires Node.js and `requirements-mcp.txt`
- Tool smoke test: `task.create` through `route_tool_call()` -> created pending job
- Credential scan: no paid API key pattern matches

## Risk

developer inspector requires Node.js and may download a free npm package. The primary verification path is now the FastAPI web UI, so `npx` is no longer required.
